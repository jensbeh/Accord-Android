...
public class ServerOfflineMemberRecyclerViewAdapter extends RecyclerView.Adapter<ServerOfflineMemberRecyclerViewAdapter.ViewHolder> {
    private Context context;
    private OnItemClickListener onItemClickListener;
    private ModelBuilder modelBuilder;
    private ArrayList<User> offlineUserList;
     ...
     */
    public ServerOfflineMemberRecyclerViewAdapter(Context context, ModelBuilder modelBuilder) {
    ...
    public void onBindViewHolder(ViewHolder viewHolder, final int position) {
        ...
        for (User offlineUser : modelBuilder.getCurrentServer().getUser()) {
            if (!offlineUser.isStatus()) {
                offlineUserList.add(offlineUser);
            }
        }

        User user = offlineUserList.get(position);
        viewHolder.userName.setText(user.getName());
        if (!user.isStatus()) {
            viewHolder.onlineStatus.setImageResource(R.drawable.offline_status_circle);
        }
    }
    ...
    public User getItem(int position) {
        ...
        return offlineUserList.get(position);
    }
}